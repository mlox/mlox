...
def colorize_text(text):
    ...
    highlighters = [
        (
            ...
            "<span style='color: black; background-color: black;'>\g<1></span>",
        ...
        (
            re.compile(r"^(\s*\|?\s*!{1}[^!].*)$", re.MULTILINE),
            bg_colors["low"],
        ),  # '!' in mlox_base.txt
        (
            re.compile(r"^(\s*\|?\s*!{2}.*)$", re.MULTILINE),
            bg_colors["medium"],
        ),  # '!!' in mlox_base.txt
        (
        ...
        (
            re.compile(
                r"(\[Plugins already in sorted order. No sorting needed!\])",
                re.IGNORECASE,
            ),
            bg_colors["green"],
        ),
        (
            re.compile(r"^(\*\d+\*\s.*\.es[mp])", re.MULTILINE),
            bg_colors["yellow"],
        ),  # Changed mod order
    ]
