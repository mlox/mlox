...
re_rule = re.compile(
    r"^\[(version|order|nearend|nearstart|conflict|note|patch|requires)((?:\s+.[^\]]*)?)\](.*)$",
    re.IGNORECASE,
)
...
re_ver_fun = re.compile(
    r"\[VER\s*([=<>])\s*%s\s*([^\]]+)\]" % plugin_version, re.IGNORECASE
)
...
re_header_version = re.compile(
    r"\b(?:version\b\D+|v(?:er)?\.?\s*)%s" % plugin_version, re.IGNORECASE
)
    ...
    if block[0:4] == b"TES3":  # Morrowind
        if len(block) < tes3_min_plugin_size:
            parse_logger.warning(
                "Cannot read plugin description(%s): file too short, returning NULL string",
                plugin,
            )
            return ""
...
class rule_parser:
    ...
    def _parse_error(self, what):
        ...
        parse_logger.error(
            "%s: Parse Error(%s), %s [Buffer=%s]"
            % (self._where(), self.curr_rule, what, self.buffer)
        )
        self.buffer = ""
    ...
    def _filename_to_regex(plugin: str):
        ...
        * matches any number of characters
        ...
        pat = "^%s$" % re_escape_meta.sub(
            r"\\\1", plugin
        )  # Start constructing the regex, but can't just use re.escape
        pat = re_plugin_meta.sub(r".\1", pat)  # handle * and ? expansion
        # handle <ver> expansion
        pat = re_plugin_metaver.sub(
            "<VER>", pat
        )  # Make sure anything like <ver> is replaced by <VER>
        pat = pat.replace(
            "<VER>", plugin_version
        )  # Work around for parsing not liking '\d' in replacement since 3.6
        return pat
    ...
    def _parse_ordering(self, rule):
        ...
        if rule == "ORDER":
            if n_order == 0:
                ...
                parse_logger.warning(
                    "%s: ORDER rule skipped because it only has one entry: %s"
        ...
        if match:
            ...
            for xp in expanded:
                ...
                if match:
                    ...
                    parse_logger.debug(
                        "parse_ver (header) version(%s) = %s (%s)"
                        % (plugin_t, p_ver_orig, p_ver)
                    )
                else:
                    match = re_filename_version.search(plugin)
                    if match:
                        ...
                        p_ver = format_version(p_ver_orig)
                        parse_logger.debug(
                            "parse_ver (filename) version(%s) = %s (%s)"
                            % (plugin_t, p_ver_orig, p_ver)
                        )
                    else:
                        ...
                        return (False, expr)
                parse_logger.debug(
                    "parse_ver compare  p_ver=%s %s ver=%s" % (p_ver, op, ver)
                )
                result = True
                if op == "=":
                    result = p_ver == ver
        ...
        if match:
            ...
            for xp in expanded:
                ...
                if bang == "!":
                    bool = not bool
                parse_logger.debug(
                    "parse_desc [DESC] returning: (%s, %s)" % (bool, expr)
                )
                if bool:
                    self.parse_dbg_indent = self.parse_dbg_indent[:-2]
        ...
        if match:
            ...
            for xp in expanded:
                ...
                if bang == "!":
                    bool = not bool
                parse_logger.debug(
                    "parse_size [SIZE] returning: (%s, %s)" % (bool, expr)
                )
                self.parse_dbg_indent = self.parse_dbg_indent[:-2]
                if bool:
                    self.parse_dbg_indent = self.parse_dbg_indent[:-2]
    ...
    def _parse_mwselua(self):
        ...
        if match:
            ...
            if len(expanded) == 1:
                ...
                parse_logger.debug(
                    'parse_mwselua [MWSE-LUA] "%s" not active' % plugin_name
                )
                self.parse_dbg_indent = self.parse_dbg_indent[:-2]
                return (False, expr)  # file does not exist
            if self.datadir == None:
            ...
            for xp in expanded:
                ...
                plugin_t = self.name_converter.truename(plugin)
                bool = os.path.exists(
                    "%s\\MWSE\\mods\\%s\\main.lua" % (self.datadir.dir, pat)
                )
                if bang == "!":
                    bool = not bool
                parse_logger.debug(
                    "parse_mwselua [MWSE-LUA] returning: (%s, %s)" % (bool, expr)
                )
                if bool:
                    self.parse_dbg_indent = self.parse_dbg_indent[:-2]
    ...
    def _parse_expression(self, prune=False):
        ...
        if self.buffer == "":
            if self._readline():
                if re_rule.match(self.buffer):
                    parse_logger.debug(
                        "parse_expression new line started new rule, returning None"
                    )
                    self.parse_dbg_indent = self.parse_dbg_indent[:-2]
                    return (None, None)
                self.buffer = self.buffer.strip()
        ...
        if match:
            ...
            parse_logger.debug(
                'parse_expression parsing expression: "%s"' % self.buffer
            )
            p = match.span(0)[1]
            self.buffer = self.buffer[p:]
            ...
            if fun == "ANY":
                ...
                if prune:
                    exprs = [
                        e
                        for e in exprs
                        if not (isinstance(e, str) and e[0:8] == "MISSING(")
                    ]
                self.parse_dbg_indent = self.parse_dbg_indent[:-2]
                self.parse_dbg_indent = self.parse_dbg_indent[:-2]
    ...
    def _parse_statement(self, rule, msg, expr):
        ...
        if rule == "CONFLICT":  # takes any number of exprs
            exprs = []
            parse_logger.debug(
                "before conflict parse_expr() expr=%s line=%s" % (expr, self.buffer)
            )
            ...
            while bool != None:
                ...
                (bool, expr) = self._parse_expression()
                parse_logger.debug(
                    "conflict parse_expr()N bool=%s bool=%s"
                    % ("True" if bool else "False", expr)
                )
            if len(exprs) > 1:
            ...
            if bool1 == None:
                parse_logger.warning(
                    "%s: PATCH rule invalid first expression" % (self._where())
                )
                self.parse_dbg_indent = self.parse_dbg_indent[:-2]
                return
            ...
            if bool2 == None:
                parse_logger.warning(
                    "%s: PATCH rule invalid second expression" % (self._where())
                )
                self.parse_dbg_indent = self.parse_dbg_indent[:-2]
                return
            ...
            if bool1 == None:
                parse_logger.warning(
                    "%s: REQUIRES rule invalid first expression" % (self._where())
                )
                self.parse_dbg_indent = self.parse_dbg_indent[:-2]
                return
            (bool2, expr2) = self._parse_expression()
            if bool2 == None:
                parse_logger.warning(
                    "%s: REQUIRES rule invalid second expression" % (self._where())
                )
                self.parse_dbg_indent = self.parse_dbg_indent[:-2]
                return
            if bool1 and not bool2:
        ...
        while True:
            ...
            if new_rule:  # start a new rule
                ...
                if self.curr_rule == "VERSION":
                    ...
                    parse_logger.info(
                        '"{0}" Version {1}'.format(
                            os.path.basename(self.rule_file), self.version
                        )
                    )
                    ...
                    self._parse_statement(
                        self.curr_rule, new_rule.group(2), new_rule.group(3)
                    )
                else:
