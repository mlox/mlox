...
from mlox.resources import base_file, user_files
...
class loadorder:
    ...
    def get_active_plugins(self):
        ...
        # Get all the plugins
        configFiles = configHandler.configHandler(
            self.plugin_file, self.game_type
        ).read()
        ...
        order_logger.info(
            'Found {0} plugins in: "{1}"'.format(len(self.order), self.plugin_file)
        )
        if self.order == []:
            order_logger.warning(
                "No active plugins, defaulting to all plugins in Data Files directory."
            )
            self.get_data_files()
    ...
    def get_data_files(self):
        ...
        order_logger.info(
            'Found {0} plugins in: "{1}"'.format(len(self.order), self.datadir)
        )
    ...
    def listversions(self):
        ...
        for p in self.order:
            ...
            out += "{0:20} {1:20} {2}\n".format(
                str(file_ver), str(desc_ver), self.caseless.truename(p)
            )
        return out
    ...
    def add_current_order(self, graph):
        ...
        for curr_i in range(1, len(self.order)):
            ...
            if (
                self.order[curr_i] not in graph.nearstart
                and self.order[curr_i] not in graph.nearend
            ):
                ...
                for i in range(prev_i, 0, -1):
                    if (
                        self.order[i] not in graph.nearstart
                        and self.order[i] not in graph.nearend
                    ):
    ...
    def get_original_order(self):
        ...
        for n in range(1, len(self.order) + 1):
            formatted.append(
                "{0:0>3} {1}".format(n, self.caseless.truename(self.order[n - 1]))
            )
        return formatted
    ...
    def get_new_order(self):
        ...
        for i in range(0, len(self.new_order)):
            ...
            if (orig_index[curr] - 1) > i:
                highlight = "*"
            formatted.append(
                "%s%03d%s %s" % (highlight, orig_index[curr], highlight, p)
            )
            if highlight == "*":
    ...
    def explain(self, plugin_name, base_only=False):
        ...
        if not base_only:
            self.add_current_order(
                plugin_graph
            )  # tertiary order "pseudo-rules" from current load order
    ...
    def update(self, progress=None):
        ...
        if not parser.read_rules(base_file, progress):
            order_logger.error(
                "Unable to parse 'mlox_base.txt', load order NOT sorted!"
            )
            self.new_order = []
            return False
        ...
        if len(self.new_order) != len(self.order):
            order_logger.error(
                "sanity check: len(self.new_order %d) != len(self.order %d)",
                len(self.new_order),
                len(self.order),
            )
            self.new_order = []
        ...
        if self.datadir:
            ...
            configHandler.configHandler(new_loadorder_output, "raw").write(
                self.new_order
            )
    ...
    def write_new_order(self):
        ...
        if isinstance(self.plugin_file, str):
            if configHandler.configHandler(self.plugin_file, self.game_type).write(
                self.new_order
            ):
                self.is_sorted = True
